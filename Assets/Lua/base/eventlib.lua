...
function _M:Connect(handler,obj)
    ...
    if obj then
        if (not self.handlerObjs[handler])then
            self.handlerObjs[handler] = {}
        end
        local inside = false
        for i,v in ipairs(self.handlerObjs[handler]) do
            if (v == obj)then
                inside = true
                break
            end
        ...
        if (not inside)then
            table.insert(self.handlerObjs[handler],obj)
        end
    end
...
function _M:Disconnect(handler,obj)
    assert(self ~= nil and type(self) == "table", "Invalid Event (make sure you're using ':' not '.')")
    ...
    if not handler then
        ...
        for k, v in pairs(self.handlers) do
            if v == handler then
                if (not obj)then
                    self.handlers[k] = nil
                    self.handlerObjs[v] = nil
                else
                    for _i,_obj in ipairs(self.handlerObjs[v]) do
                        if (_obj == obj)then
                            table.remove(self.handlerObjs[v],_i)
                            break
                        end
                    end
                    if (#self.handlerObjs[v] == 0)then
                        self.handlers[k] = nil
                        self.handlerObjs[v] = nil
                    end
                end
...
function _M:Fire(...)
    ...
    for k, v in pairs(self.handlers) do
        i = i + 1
        local objs = self.handlerObjs[v]
        if (objs and #objs > 0)then
            for _i,obj in ipairs(objs) do
                spawn(function() 
